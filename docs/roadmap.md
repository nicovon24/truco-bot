...
- **Fase 1 — parcial.** Hecho: workspace `uv` (`engine`, `api`, `training`), Ruff, mypy strict, pytest, esqueleto de carpetas, `.gitignore`/`.gitattributes`. Pendiente: workspace `pnpm` + scaffold Next.js (ESLint, TypeScript strict, Tailwind, Zustand, Vitest, `openapi-typescript`), export de OpenAPI (`api/openapi.json`), Dockerfiles + `docker-compose.yml`, CI de GitHub Actions.
...
- Skills de front instaladas para Claude y Codex: `impeccable`. Pendiente: `ui-ux-pro-max`.
